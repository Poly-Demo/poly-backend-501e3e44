...
import { products, type NewProductRow } from './schema.js';

/**
 * The original PolyShopping catalog. Previously hardcoded in
 * `src/products/repository.ts`; now the single source of truth for seeding.
 */
...
/**
 * Idempotent seed: inserts the catalog, skipping rows whose `id` already
 * exists (`ON CONFLICT DO NOTHING`), so re-running is always safe.
 */
async function main() {
  ...
  console.log(
    `[seed] done — ${inserted.length} new row(s) inserted, ${
      seedProducts.length - inserted.length
    } already present`
  );
  await pool.end();
}
